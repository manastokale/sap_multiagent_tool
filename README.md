...
flowchart TB
    ...
    LLM["Gemini or Groq client"] -. "live mode" .-> Agents
...
`generate`, `evaluate`, and `analyze` show Rich progress bars so long live runs expose
which conversation is currently being processed and how much of the file is complete.
...
ToolGen is runnable without credentials. When keys are present, it can use Gemini and
Groq for planning, user simulation, assistant turns, and judging. The default live
profile is `full`, which is the most faithful multi-agent simulation. For quota-limited
runs, `hybrid` keeps planner, assistant tool-decision turns, and judge live while using
deterministic user turns and deterministic tool-result summaries.

```bash
cp .env.example .env
# Set GEMINI_API_KEY and/or GROQ_API_KEY in .env

export TOOLGEN_LLM_PROVIDER=groq
export TOOLGEN_GENERATION_MODEL=llama-3.1-8b-instant
export TOOLGEN_JUDGE_MODEL=llama-3.1-8b-instant
export TOOLGEN_RANDOMIZE_MODELS=false
toolgen generate -n 10 --seed 42 -o output/live_sample.jsonl
```

Quota-saving hybrid live mode:
...
For assessment-style pure live runs, fail instead of falling back:
...
- Explicit `TOOLGEN_GENERATION_MODEL` and `TOOLGEN_JUDGE_MODEL` values are respected
  even when randomization is enabled. To randomize, set non-empty
  `TOOLGEN_GENERATION_MODEL_POOL` and `TOOLGEN_JUDGE_MODEL_POOL` values.
...
judge output, repair flags, and the sampled tool path.
